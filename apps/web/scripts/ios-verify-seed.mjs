...
const t = (m, p, tok, b, l) => api(m, p, tok, b).catch((e) => { console.warn(`  ! ${l || p}: ${String(e.message).slice(0, 120)}`); return null; });
...
const prog = await api("POST", "/v1/programs", tok, { name: "Hypertrophy Block", description: "Upper/lower", goal: "hypertrophy" });
let n = 0;
for (const [name, rest] of [["Push", false], ["Pull", false], ["Legs", false], ["Rest", true], ["Upper", false], ["Lower", false], ["Rest", true]]) {
  ...
  if (!rest && list.items.length) for (let k = 0; k < 4; k++)
    await t("POST", `/v1/program-slots/${s.id}/exercises`, tok, { exercise_id: ex(n++), target_sets: 4, target_reps_low: 8, target_reps_high: 12, rest_seconds: 120, rep_mode: "range" }, "ex");
}
await t("PATCH", `/v1/programs/${prog.id}`, tok, { periodization_mode: "block", mesocycle_length_microcycles: 4, auto_deload: true, intensity_mode: "rpe" }, "patch");
await t("POST", `/v1/programs/${prog.id}/activate`, tok, null, "activate");

// body weights -> Health Metrics
for (let d = 20; d >= 0; d -= 5) await t("POST", "/v1/body-metrics", tok, { weight_kg: 82 - (20 - d) * 0.1 }, "weight");
